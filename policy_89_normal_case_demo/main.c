...
{
    vnn_ReleasePolicy89( graph, TRUE );
...
{
    return vnn_PostProcessPolicy89( graph );
}
    ...
    )
{
    ...
    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessPolicy89_ImageProcess(graph, inputs, input_num);
    }
    */
    return vnn_PreProcessPolicy89( graph, inputs, input_num );
}
    ...
    )
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreatePolicy89( data_file_name, NULL,
